	...
	int unsupType)
{
	...
	for(ii=0;ii<bigram.Size();ii++)
	{
		...
		if(unsupType%10==3)//PPMI BPE
			scoreppmi = bigramPPMI[ii]* weight;
		...
		if(unsupType%10==4)//AV+DLG-BPE
		{
			scoredlg=(double)bigramAV1[ii].sizeTrue();
			if(score>(double)(bigramAV2[ii].sizeTrue()))
				scoredlg=(double)(bigramAV2[ii].sizeTrue());
			scoredlg = ((1.0-lamda)*bigramDLG[ii]+lamda*scoredlg)* weight;
		}
...
void BPETrain(char *in1, int a, int unsupType, size_t lexSize, char *out1)
{
	...
	if(unsupType%10==3) fprintf(stderr,"PPMI BPE ...\n");
	if(unsupType%10==4) fprintf(stderr,"AV+DLG BPE ...\n");
	...
	for(ii=0;ii<lexSize;ii++)
	{
		...
		if(unsupType>=10 && unsupType<=20) weight=1.0;
		...
		if(unsupType%10==3) scoremax = bigramPPMI[id2];
		...
		while(jj<bigramPosList[id2].size())
		{
			...
			for(mm=0;mm<mvc.size();mm++)
			{
				...
				if(mvc[mm]>0)
				{
					...
					if(unsupType%10==3) updatePPMI(unigramDLG,bigram,vobseg[mvc[mm]-1],vobseg[mvc[mm]],bigramPPMI);
				...
				if(mvc[mm]+1<vobseg.size())
				{
					...
					if(unsupType%10==3) updatePPMI(unigramDLG,bigram,vobseg[mvc[mm]],vobseg[mvc[mm]+1],bigramPPMI);
